...
助教在隔离工作区内运行，用户可以在真实终端中查看和手动终止。稳定写出的 `result-manifest.json` 是成果交付信号，交互式终端无需退出；助教完成自验收后，Go 服务只负责把其声明的目录原子发布为学习资产，并记录可恢复的运行状态。即使页面刷新、服务重启或终端结果晚到，任务也可以恢复和幂等补提交。
