...
| 5 — assistant and CLI | in progress | Task storage/dispatch, execution, registry, runtime selection, prompt assembly, and CLI launch implementations moved below `modules/assistant/internal` with a single root facade; the now-confirmed launcher-only artifact promotion code moved from assets into assistant launch. All old flat imports were removed. Interim gate: archcheck 42 packages/120 direct edges and `go test ./... -count=1` PASS. Dispatcher decomposition, consumer ports, and the fake-receiver Windows smoke remain before this wave is delivered. |
