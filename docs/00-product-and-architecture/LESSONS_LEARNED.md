...
## 20. 助教产物不能再经过第二套服务端验收

### 症状

助教在可见 CLI 中已经完成自验收的文档和图，但服务端又因清单、哈希或任务类型规则
拒绝发布，教师任务显示失败，资产页“助教成果”为空。

### 根因

外部助教已经是输出质量的权威，却又要求 Go 对同一成果做第二次内容验收。这造成了
职责重叠，并把一个本应是存储操作的发布流程扩大成另一套任务生命周期。

### 铁律

- **助教完成自验收后，LLL 只负责发布**：不要重复检查文件清单、哈希、内容或任务类型。
- attempt 工作区仍然必须隔离，正式资产仍由 Go 原子写入；这是存储边界，不是内容验收。
- 发布失败应明确指出未写入的输出并保持终态，不能用服务端再校验或自动重跑替代助教决策。
- 回归测试应验证没有文件清单、字节数或哈希的成果目录会被完整发布。
