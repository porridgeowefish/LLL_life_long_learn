...
## 18. CSS Modules 不会隔离裸标签选择器（懒加载页面样式污染）

### 症状
先访问 Token 用量页，再进入教师对话，Markdown 列表项变成白色三列卡片；刷新教师页后恢复正常。

### 根因
`UsagePage.module.css` 使用了顶层 `li`、`ol`、`h1` 等标签选择器。CSS Modules 只重写局部类名，不会给裸标签加作用域。用量页作为懒加载路由被访问后，其样式表留在当前 SPA 文档中，随后污染教师回复；直接刷新教师页时没有加载用量页样式，所以现象暂时消失。

### 铁律
- **CSS Module 中的标签选择器必须挂在局部类下面**，例如 `.list li`，不允许用顶层 `li` 表达组件内部样式。
- **“换页后异常、刷新恢复”要检查懒加载样式残留**：按访问顺序复现，并对目标元素读取 computed style 与命中规则。
- 跨路由样式回归测试应真实加载相关 CSS，再断言组件外元素没有获得该布局、背景等可见属性。

---

## 19. Gitignored 用户文件不能把“留在原地”当成升级保障（偏好丢失）

### 症状
版本更新或启动方式变化后，偏好页显示模板或空内容；重新填写后暂时正常。

### 根因
`preferences.md` 位于仓库兼工作区根目录且被 Git 忽略，没有恢复副本。目录替换或文件丢失后，偏好页的 `Ensure` 会创建默认模板。与此同时，主程序在 Go 包初始化后才处理 `--workspace`，只更新环境变量却没有重算 `WORKSPACE/PROJECTS_ROOT/AGENTS_ROOT`，不同启动方式可能读写不同位置。更直接的清空来源是路由契约测试：它原本按每条路由的真实 HTTP 方法发请求，探测 `PUT /api/preferences` 时用空 body 覆盖了真实文件，因此发布前运行测试就会稳定清空偏好。

### 铁律
- **Gitignored 只代表“不提交”，不代表“升级时持久”**。学习者维护且不可重建的数据必须有仓库外恢复副本。
- **路由注册测试不能用真实写方法探测 handler**。使用不会匹配业务路由的方法，并把运行时工作区切到测试临时目录，双重阻断副作用。
- 恢复副本有内容而主文件意外变成 0 字节时，启动和编辑器读取必须恢复副本，不能用空文件反向覆盖有效备份；用户显式保存空内容时两者会同时为空，仍被尊重。
- **派生路径必须在配置优先级解析完成后统一应用**；进程启动后修改环境变量不会自动更新已初始化的 Go 变量。
- 自动恢复不能制造第二个可编辑事实源：工作区文件仍是 canonical，仓库外副本只用于缺失恢复。

## 20. 助教“已生成”必须以资产库和任务状态共同为准

### 症状

助教在可见 CLI 中已经写出了文档和图，但教师任务显示失败，资产页“助教成果”为空；
偶尔又会出现文件已经落入 `assets/generated/`、任务却仍显示失败的相反分叉。

### 根因

生成、校验、提升到 canonical `assets/generated/`、更新任务状态是多个持久化步骤。
旧逻辑把提升错误压缩成通用 `commit-failed`，恢复路径只处理进程丢失和无效清单，
有效成果因此永远留在 attempt 沙箱。一次提升边界的短暂失败还可能让文件操作与任务
状态先后落盘，形成“文件有、状态错”的状态。

### 铁律

- **“模型写出了文件”不等于“学习者拥有成果”**：必须同时检查已验证的
  `assets/generated/<artifact-id>/artifact.json`、`GET /generated` 和 task 状态。
- 恢复只能重放**幂等的 canonical 提升**，不能重跑模型/CLI，更不能把所有失败任务
  自动重试；把资格限制为 sealed、哈希有效、独立 deliverable、且 core assets 未变的情况。
- 每个失败投影都要指出未提交的输出类别；不要丢弃唯一能缩小排查范围的错误上下文。
- 对账要双向收敛：文件存在且 provenance 匹配时修正状态，文件不存在时才执行受限提升。
- 回归测试必须含一个已完成失败 `commit.json`，并覆盖“artifact 已存在、状态仍失败”的
  崩溃窗口。
