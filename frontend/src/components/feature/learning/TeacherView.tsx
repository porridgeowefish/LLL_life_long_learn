...
export function TeacherView({ slug, title }: { slug: string; title: string }) {
  ...
  const activeTasks = (tasksQuery.data ?? []).filter((task) => task.status === 'queued' || task.status === 'running');
  ...
  }, [conversation.data?.messages.length, live?.learner.id, live?.text, live?.teacherId, live?.notice]);
  ...
  useEffect(() => {
    ...
    void resumeTeacherTurn(slug, next.signal, (frame) => {
      ...
      if (found) void queryClient.invalidateQueries({ queryKey: ['learning-workspace', slug] });
    }).catch((cause) => {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      flushDeltas();
      await queryClient.invalidateQueries({ queryKey: ['learning-workspace', slug] });
      setLive(null);
    } catch (cause) {
  ...
  for (const link of conversation.data?.taskLinks ?? []) {
    linksByMessage.set(link.messageId, [...(linksByMessage.get(link.messageId) ?? []), link.taskId]);
  }
  ...
  return (
    <section className={s.teacher}>
      ...
      <div className={s.transcript} ref={transcriptRef} role="log" aria-label="教师对话" aria-live="polite" onScroll={() => {
      ...
      <aside className={s.dataRail} aria-label="学习单元数据">
        ...
        <dl>
          <div><dt>对话</dt><dd>{conversation.data?.messages.length ?? 0}<small> 条消息</small></dd></div>
...
function Message({ slug, message, tasks, sourceNames, notice, streaming = false }: { slug: string; message: ConversationMessage; tasks: AssistantTask[]; sourceNames: Map<string, string>; notice?: string; streaming?: boolean }) {
    ...
    <article className={teacher ? s.teacherMessage : s.learnerMessage}>
      ...
      <div className={s.messageBody}>
        ...
        {(message.blocks ?? []).map((block) => block.type === 'reasoning-summary' ? (
          <details key={block.id} className={s.reasoning}>
            <summary>思考过程</summary>
            <MarkdownView source={block.source ?? ''} />
          </details>
        ) : block.type === 'markdown' ? <MarkdownView key={block.id} source={block.source ?? ''} className={s.markdown} /> : block.type === 'attachment' ? <span key={block.id} className={s.attachment}>资料 · {sourceNames.get(block.artifactRef ?? '') ?? '已选择资料'}</span> : null)}
...
function TaskCard({ slug, task }: { slug: string; task: AssistantTask }) {
  ...
  return (
    <div className={s.taskCard} data-status={task.status}>
      ...
      {!!task.result?.deliverables?.length && <div className={s.deliverables}>{task.result.deliverables.map((artifactId) => <a key={artifactId} href={`/api/projects/${encodeURIComponent(slug)}/generated/${encodeURIComponent(artifactId)}/open`} target="_blank" rel="noreferrer">查看助教成果</a>)}</div>}
